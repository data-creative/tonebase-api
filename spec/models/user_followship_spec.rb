...
RSpec.describe UserFollowship, type: :model do
  ...
  describe "validations" do
    ...
    context "uniqueness" do
      subject { build(:user_followship) }
      it { should validate_uniqueness_of(:followed_user).scoped_to(:user_id) }
    end
    ...
    describe "directional_uniqueness" do
      context "when a one user is following another" do
        ...
        it "should not allow the same user to follow the same other" do
          expect{ duplicate_followship }.to raise_error(ActiveRecord::RecordInvalid, "Validation failed: Followed user has already been taken")
        end
